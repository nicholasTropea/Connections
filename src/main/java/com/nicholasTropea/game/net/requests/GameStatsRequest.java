...
/**
 * Concrete request for retrieving statistics about a game.
 * 
...
public class GameStatsRequest extends Request {
    ...
    /**
     * Factory method that prompts the user to select a game stats query type
     * and collects the necessary parameters interactively.
     * 
     * @param scan The Scanner to read user input from
     * @return A new GameStatsRequest configured based on user selection
     */
    public static GameStatsRequest createRequest(Scanner scan) {
        int selection = getSelection(scan);

        switch (selection) {
            case 1:
                return new GameStatsRequest(true);
            case 2:
                int id = getValidGameID(scan);
                return new GameStatsRequest(id);
            default:
    ...
    /**
     * Displays the game stats menu and prompts the user to select an option.
